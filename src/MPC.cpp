...
// COST WEIGHTS
const double W_cte    =    150000.0;
const double W_epsi   =   6000000.0;
const double W_v      =       800.0;
const double W_delta  =      1000.0;
const double W_a      =         0.0;
const double W_ddelta = 100000000.0;
...
double ref_v = 40 * 0.44704; // conversion from miles per hour to meters per second
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = a_start; i < n_vars; i++) {
    vars_lowerbound[i] = -3.5;
    vars_upperbound[i] = 3.5;
  }
  ...
  // Storing the predicted trajectory so that it can be accessed from the controller
  this->mpc_x_vals = {};
  this->mpc_y_vals = {};
  for (int i = 0; i < N; i++) {
